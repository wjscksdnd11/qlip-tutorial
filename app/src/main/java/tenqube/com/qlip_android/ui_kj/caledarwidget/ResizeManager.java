...
public class ResizeManager {
     ...
     */
    @NonNull
    private CalendarWidgetView mCalendarView;
    ...
    public ResizeManager(@NonNull CalendarWidgetView calendarView) {
    ...
    public boolean onInterceptTouchEvent(@NonNull MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                Log.i("TouchEvent", "onIntercepttouchEvent  - MontionEvent.Action_move" + checkForResizing(ev));
                return checkForResizing(ev);

            case MotionEvent.ACTION_CANCEL:
            case MotionEvent.ACTION_UP:
                Log.i("TouchEvent", "onIntercepttouchEvent  - action_up");
                finishMotionEvent();
                return false;
        }
        ...
        if (mState == State.DRAGGING) {
            switch (action) {
                case MotionEvent.ACTION_MOVE:
                    ...
                    Log.i("TouchEvent", "onTouchEvent  - action_move , "+deltaY);
                    mProgressManager.applyDelta(deltaY);
                    break;
                case MotionEvent.ACTION_CANCEL:
                case MotionEvent.ACTION_UP:
                    Log.i("TouchEvent", "onTouchEvent  - action_up");
            ...
            Log.i("TouchEvent", "onTouchEvent  - else_if action_move");
    ...
    public boolean checkForResizing(MotionEvent ev) { // FIXME this method should only return true / false. Make another method for starting animation
        ...
        Log.i("checkForresizing", Math.abs(yDIff) + " , " + mTouchSlop);
...
//                    mCalendarView.populateLayout();
                ...
                Log.i("state",(state == CalendarManager.State.MONTH)+"");
    ...
    private void finishMotionEvent() {
        if (mProgressManager != null && mProgressManager.isInitialized()) {
            Log.i("finishMotionEvent", "finishMotionEvent");
            startScolling();
        }
    }
    ...
    private int calculateDistanceForDrag(MotionEvent event) {
        Log.i("calculateDistanceFordrag",event.getY()+" , "+mDragStartY);
